  ...
  return (
    <div className="relative w-full mt-15 flex flex-col items-center justify-center">
      <div className="w-full flex flex-col items-center justify-center text-center">
        <div className="text-[24px] font-medium text-[#555555] max-w-[750px]">
          Чем точнее запрос — тем лучше помощь
        </div>
        <div className="text-[40px] font-semibold text-gray-900 max-w-[827px] mt-2">
      ...
      >
        <div
          className="absolute top-[-100px] left-1/2 -translate-x-1/2 w-[1000px] h-[100px] opacity-30 pointer-events-none"
          style={{
        ...
        />
        <div
          className="absolute bottom-[-100px] left-1/2 -translate-x-1/2 w-[1000px] h-[120px] opacity-30 pointer-events-none"
          style={{
        ...
        {!isMobile && (
          <button
            onClick={handlePrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-50 hover:opacity-80"
          >
        ...
        <div className="relative w-full h-full flex items-center justify-center overflow-visible z-10">
          {chatSamples.map((chat, i) => {
            ...
            if (Math.abs(diff) > 3) return null; // Ограничение видимых карточек

            const position = diff;
            const scale = 1 - Math.abs(position) * scaleStep;
            const translateX = position * offsetX;
            const zIndex = 30 - Math.abs(position) * 10;
            const opacity = Math.abs(position) <= 2 ? (position === 0 ? 1 : 0.85) : 0;
            const pointerEvents = Math.abs(position) <= 2 ? "auto" : "none";

            return (
              <div
                key={i}
                className={`absolute rounded-[28px] border border-gray-200 bg-white px-4 pt-4 pb-3 shadow-sm flex flex-col justify-between transition-all duration-300 ease-in-out`}
                style={{
                  width: baseWidth,
                  height: baseHeight,
        ...
        {!isMobile && (
          <button
            onClick={handleNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-50 hover:opacity-80"
          >
...
const CardContent = ({ chat, position = 0 }) => (
  <>
    <div className="flex items-start gap-2">
      <div className="bg-blue-100 rounded-full w-6 h-6 flex items-center justify-center mt-[2px]">
    ...
    <div className="w-full mt-4 px-3 py-2 border border-gray-200 rounded-[20px] bg-white shadow-inner flex flex-col justify-between text-left min-h-[88px] relative group">
      <div className="text-[12px] text-[#555555] font-[Manrope] leading-tight mb-2">
        {chat.user}
      </div>
