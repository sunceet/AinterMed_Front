      ...
      <div className="flex flex-col gap-6 items-center max-w-[1080px] w-full">
        ...
        {faqs.map((faq, index) => {
          ...
          return (
            <div
              key={index}
              className="w-full bg-white  border border-white xl:rounded-[32px] rounded-[18px] overflow-hidden transition-all duration-500 ease-in-out"
            >
              {/* ВОПРОС */}
              <div
                className="px-4 sm:px-6 py-6 flex items-center justify-between gap-1 cursor-pointer min-h-[72px] md:min-h-[96px]"
                onClick={() => toggle(index)}
              >
                <h3 className="flex-1 text-left text-[13px] sm:text-[18px] md:text-[24px] leading-snug text-black font-semibold font-[Involve]">
                  {faq.question}
                </h3>
                ...
                <div className="w-10 h-10 sm:w-[56px] sm:h-[56px] perspective shrink-0 flex items-center justify-center">
                  ...
                  >
                    ...
                    <div className="flip-card-back absolute inset-0 flex items-center justify-center bg-[#F3F3F3] rounded-xl rotate-x-180">
                      <div className="relative w-4 h-4 sm:w-6 sm:h-6">
                        <span className="absolute left-0 top-1/2 w-full h-[2px] bg-black transform -translate-y-1/2 rotate-45 origin-center" />
                        <span className="absolute left-0 top-1/2 w-full h-[2px] bg-black transform -translate-y-1/2 -rotate-45 origin-center" />
                      </div>
                    </div>
                  </div>
