...
const ReviewCard = ({ review }) => (
  <div className="w-full max-w-[350px] h-[380px] rounded-[24px] border border-white p-5 bg-white flex-shrink-0">
    <div className="flex items-start gap-4">
      <div className="w-12 h-12 rounded-full border border" />
      <div>
...
const ReviewsGrid = () => {
  ...
  const scrollToPage = (page) => {
    ...
    container.scrollTo({
      left: (page - 1) * (cardWidth + 16) * reviewsPerPage,
      behavior: "smooth",
    });
  };

  const handlePaginationClick = (page) => {
    isManualScroll.current = false; // disable auto-scroll handling
    setCurrentPage(page);
  ...
  const handleMobileScroll = () => {
    ...
    const cardWidth = container.children[0]?.offsetWidth || 350;
    const newPage = Math.round(scrollLeft / ((cardWidth + 16) * reviewsPerPage)) + 1;
    ...
    <div className="relative flex flex-col items-center px-4 md:px-1 mt-[100px] max-w-screen-xl mx-auto">
      ...
      {!isMobile && (
        <div className="relative w-full items-center justify-center z-10 hidden md:flex">
          ...
          >
            <img src="/assets/svg/arrow_reviews.svg" alt="Left" className="w-7 h-7 select-none" />
          </button>

          <div className="flex gap-6 justify-center items-start min-h-[380px]">
          ...
          >
            <img
              ...
              alt="Right"
              className="w-7 h-7 select-none rotate-180"
            />
          </button>
        </div>
      )}
      ...
      {isMobile && (
        <div
          ...
          className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 w-full scrollbar-hidden"
        ...
        {pagination.map((item, index) =>
          item === "..." ? (
            ...
            <button
              ...
              className={`w-[38px] h-[38px] rounded-full border text-sm font-medium transition ${
                currentPage === item
